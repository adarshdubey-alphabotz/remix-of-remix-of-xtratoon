...
import {
  ...
  Plus, X, Instagram, Twitter, Check, XCircle, Wallet, Trophy, Flame, Share2, HelpCircle,
  Bug, Settings, Copy,
} from 'lucide-react';
...
const SettingsRow: React.FC<{ icon: React.ReactNode; label: string; value?: string; onClick?: () => void; danger?: boolean }> = ({ icon, label, value, onClick, danger }) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center gap-3.5 px-4 py-3.5 hover:bg-muted/40 transition-colors text-left ${danger ? 'text-destructive' : ''}`}
  >
    <span className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm ${danger ? 'bg-destructive/10 text-destructive' : 'bg-primary/8 text-primary'}`}>
      {icon}
    </span>
    <span className="flex-1 min-w-0">
      <span className={`text-sm font-medium ${danger ? 'text-destructive' : 'text-foreground'}`}>{label}</span>
      {value && <span className="block text-[11px] text-muted-foreground truncate mt-0.5">{value}</span>}
    </span>
...
const SectionHeader: React.FC<{ onBack: () => void; title: string }> = ({ onBack, title }) => (
  <div className="flex items-center gap-3 px-4 py-4 border-b border-border/30 sticky top-0 bg-background/95 backdrop-blur-sm z-10">
...
const ProfilePage: React.FC = () => {
  ...
  const renderMain = () => (
    <motion.div key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-0">
      {/* Profile Header - Clean like reference image */}
      <div className="relative px-6 pt-6 pb-6">
        ...
        <div className="flex flex-col items-center text-center">
          <div className="relative mb-3">
            ...
            <button onClick={() => { setActiveSection('edit'); setShowAvatarPicker(true); }}
              className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-lg border-2 border-background">
          ...
          {user && user.app_metadata?.email_verified !== true && (
            <button onClick={() => navigate('/verify')}
              ...
              <div className="text-left flex-1 min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground">Tap to verify</p>
              </div>
          ...
          <div className="w-full max-w-[200px] mt-4">
            <div className="w-full h-1.5 rounded-full bg-muted overflow-hidden">
              <motion.div className="h-full bg-primary rounded-full" initial={{ width: 0 }} animate={{ width: `${completionScore}%` }} transition={{ duration: 0.8 }} />
            </div>
      ...
      {isCreator && (
        <div className="px-4 pb-2">
          <button onClick={() => setActiveSection('wallet')}
            className="w-full flex items-center justify-between px-4 py-3.5 rounded-2xl border border-primary/20 bg-gradient-to-r from-primary/5 to-primary/10 hover:from-primary/10 hover:to-primary/15 transition-colors">
            <div className="flex items-center gap-3">
              <span className="w-9 h-9 rounded-xl flex items-center justify-center text-sm bg-primary/20 text-primary"><Wallet className="w-4 h-4" /></span>
              <div className="text-left">
                <span className="text-sm font-medium block">My Wallet</span>
                <span className="text-[10px] text-muted-foreground">Earnings & payouts</span>
              </div>
            </div>
      ...
      <div className="px-4 pb-2">
        <button onClick={() => setShowNotifications(!showNotifications)}
          ...
          <div className="flex items-center gap-3">
            <span className="w-9 h-9 rounded-xl flex items-center justify-center text-sm bg-primary/10 text-primary"><Bell className="w-4 h-4" /></span>
            <span className="text-sm font-medium">Notifications</span>
          </div>
          <div className="flex items-center gap-2">
            {userUnreadCount > 0 && <span className="px-2 py-0.5 text-[10px] font-bold bg-destructive text-destructive-foreground rounded-full">{userUnreadCount}</span>}
                ...
                {userNotifs.length === 0 ? (
                  ...
                  <div className="max-h-72 overflow-y-auto divide-y divide-border/30">
                    {userNotifs.map((n: any) => {
                      ...
                      return (
                        <button key={n.id} onClick={() => { markRead(n.id); if (n.reference_id && (n.type === 'new_chapter' || isApproval)) navigate(`/title/${n.reference_id}`); }}
                          className="w-full text-left px-4 py-3 hover:bg-muted/40 transition-colors flex items-start gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${isApproval ? 'bg-green-500/10' : isRejection ? 'bg-destructive/10' : 'bg-muted/60'}`}>
                            {isApproval ? <Check className="w-4 h-4 text-green-500" /> : isRejection ? <XCircle className="w-4 h-4 text-destructive" /> : <Bell className="w-4 h-4 text-primary" />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold">{n.title}</p>
                            {n.message && <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{n.message}</p>}
                            <p className="text-[10px] text-muted-foreground/70 mt-1">
                              {(() => { const s = Math.floor((Date.now() - new Date(n.created_at).getTime()) / 1000); if (s < 60) return 'just now'; if (s < 3600) return `${Math.floor(s / 60)}m ago`; if (s < 86400) return `${Math.floor(s / 3600)}h ago`; return `${Math.floor(s / 86400)}d ago`; })()}
                            </p>
                          </div>
                        </button>
                      );
      ...
      {/* Settings Groups - Clean list style like reference image 3 */}
      <div className="space-y-2 px-4 pb-6">
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          {isAdmin && adminMode ? (
            <>
              <SettingsRow icon={<Shield className="w-4 h-4" />} label="Admin Panel" value="Manage submissions & users" onClick={() => navigate('/admin')} />
              <div className="h-px bg-border/30 ml-16" />
              <SettingsRow icon={<Settings className="w-4 h-4" />} label="Admin Settings" value="Site config & moderation" onClick={() => navigate('/admin/settings')} />
            </>
          ) : (
            <>
        ...
        {isAdmin && (
          <div className="rounded-2xl border border-border bg-card overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-4 cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => setAdminMode(!adminMode)}>
              <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0"><Shield className="w-4 h-4 text-primary" /></div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium">Mode Switch</p>
                <p className="text-[11px] text-muted-foreground">{adminMode ? 'Admin Mode' : 'Creator Mode'}</p>
              </div>
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          ...
          <div className="h-px bg-border/30 ml-16" />
          <SettingsRow icon={<Bell className="w-4 h-4" />} label="Manage notifications" value={creatorAlerts ? 'On' : 'Off'} onClick={() => setCreatorAlerts(prev => !prev)} />
        </div>

        {/* Info & Support */}
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          <SettingsRow icon={<HelpCircle className="w-4 h-4" />} label="FAQ" onClick={() => navigate('/blog')} />
          <div className="h-px bg-border/30 ml-16" />
          <SettingsRow icon={<Bug className="w-4 h-4" />} label="Report a bug" onClick={() => window.open('mailto:support@komixora.fun?subject=Bug Report', '_blank')} />
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          <div className="flex items-center gap-3 px-4 py-4 cursor-pointer hover:bg-muted/30 transition-colors" onClick={toggleNightShift}>
            <div className="w-9 h-9 rounded-xl bg-orange-500/10 flex items-center justify-center flex-shrink-0"><Eye className="w-4 h-4 text-orange-500" /></div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium">Night Shift</p>
              <p className="text-[11px] text-muted-foreground">Warm color filter for night reading</p>
            </div>
  ...
  const renderEdit = () => (
    <motion.div key="edit" initial={slideVariants.enter} animate={slideVariants.center} exit={slideVariants.exit} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
      ...
      <div className="px-4 py-6 space-y-5">
        ...
        {success && <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="p-3 rounded-xl border border-primary/40 bg-primary/10 text-primary text-sm flex items-center gap-2"><CheckCircle className="w-4 h-4" />{success}</motion.div>}
        <div>
        ...
        <div className="space-y-1">
          ...
          <input value={displayName} onChange={e => setDisplayName(e.target.value)} className="w-full px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary transition-colors" placeholder="Your display name" />
        </div>
        <div className="space-y-1">
        ...
        <div className="space-y-1">
          <label className="text-sm font-medium text-muted-foreground">Username {profileType === 'publisher' && <span className="text-destructive">*</span>}</label>
          <input value={username} onChange={e => setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9_.]/g, ''))} className="w-full px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary transition-colors" placeholder="e.g. moonx.d (min 5)" />
        </div>
        <div className="space-y-1">
          <label className="text-sm font-medium text-muted-foreground">Bio</label>
          <textarea value={bio} onChange={e => setBio(e.target.value)} rows={3} className="w-full px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary transition-colors resize-none" placeholder="Tell readers about yourself..." />
        </div>
    ...
    return (
      <motion.div key="social" initial={slideVariants.enter} animate={slideVariants.center} exit={slideVariants.exit} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
        ...
        <div className="px-4 py-6 space-y-4">
          <p className="text-sm text-muted-foreground">Add your social profiles so readers can find you.</p>
          {SOCIAL_PLATFORMS.map(({ key, label, icon, placeholder }) => (
            <div key={key} className="space-y-1">
              <label className="text-sm font-medium text-muted-foreground flex items-center gap-2"><span>{icon}</span> {label}</label>
              <div className="flex gap-2">
                <input value={socialLinks[key] || ''} onChange={e => updateSocialLink(key, e.target.value)} className="flex-1 px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary transition-colors" placeholder={placeholder} />
                {socialLinks[key] && <button onClick={() => removeSocialLink(key)} className="w-10 h-10 rounded-xl border border-border hover:bg-destructive/10 hover:text-destructive flex items-center justify-center transition-colors self-end"><X className="w-4 h-4" /></button>}
              </div>
            </div>
          ))}
          {customLinks.map(([key, value]) => (
            <div key={key} className="space-y-1">
              <label className="text-sm font-medium text-muted-foreground flex items-center gap-2"><span>🔗</span> {key.replace('custom_', '').replace(/_/g, ' ')}</label>
              <div className="flex gap-2">
                <input value={value || ''} onChange={e => updateSocialLink(key, e.target.value)} className="flex-1 px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary transition-colors" />
                <button onClick={() => removeSocialLink(key)} className="w-10 h-10 rounded-xl border border-border hover:bg-destructive/10 hover:text-destructive flex items-center justify-center transition-colors self-end"><X className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
          <div className="border-t border-border pt-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <input value={customLinkName} onChange={e => setCustomLinkName(e.target.value)} className="px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary" placeholder="Link name" />
              <input value={customLinkUrl} onChange={e => setCustomLinkUrl(e.target.value)} className="px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary" placeholder="URL" />
            </div>
            <button onClick={addCustomLink} disabled={!customLinkName.trim() || !customLinkUrl.trim()} className="px-4 py-2 rounded-xl border border-primary text-primary text-sm font-medium hover:bg-primary/10 transition-colors disabled:opacity-40">Add Link</button>
          </div>
          <button onClick={handleSave} disabled={saving} className="w-full py-3.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg">
  ...
  const renderLocation = () => (
    <motion.div key="location" initial={slideVariants.enter} animate={slideVariants.center} exit={slideVariants.exit} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
      ...
      <div className="px-4 py-6 space-y-4">
        <div className="space-y-1"><label className="text-sm font-medium text-muted-foreground">Continent</label><select value={continent} onChange={e => { setContinent(e.target.value); setCountry(''); }} className="w-full px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary"><option value="">Select...</option>{continents.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
        <div className="space-y-1"><label className="text-sm font-medium text-muted-foreground">Country</label><select value={country} onChange={e => setCountry(e.target.value)} disabled={!continent} className="w-full px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary disabled:opacity-50"><option value="">Select...</option>{availableCountries.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
        <div className="space-y-1"><label className="text-sm font-medium text-muted-foreground">Timezone</label><select value={timezone} onChange={e => setTimezone(e.target.value)} className="w-full px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary"><option value="">Select...</option>{timezones.map(tz => <option key={tz} value={tz}>{tz}</option>)}</select></div>
        <div className="space-y-1"><label className="text-sm font-medium text-muted-foreground">Currency</label><select value={currency} onChange={e => setCurrency(e.target.value)} className="w-full px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary"><option value="">Select...</option>{currencies.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
        <button onClick={handleSave} disabled={saving} className="w-full py-3.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg"><Save className="w-4 h-4" /> {saving ? 'Saving...' : 'Save Location'}</button>
      </div>
    </motion.div>
  );
      ...
      <div className="px-4 py-6 space-y-5">
        ...
        <div className="space-y-1">
          ...
          <div className="relative">
            <input type={showPassword ? 'text' : 'password'} value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full px-4 py-3 pr-11 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary transition-colors" placeholder="At least 6 characters" />
            <button onClick={() => setShowPassword(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">{showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>
          </div>
        </div>
        <div className="space-y-1">
          <label className="text-sm font-medium text-muted-foreground">Confirm Password</label>
          <input type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full px-4 py-3 bg-card border border-border rounded-xl text-sm focus:outline-none focus:border-primary transition-colors" placeholder="Repeat password" />
        </div>
        <button onClick={handlePasswordChange} disabled={passSubmitting} className="w-full py-3.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg"><Lock className="w-4 h-4" /> {passSubmitting ? 'Updating...' : 'Change Password'}</button>
        <div className="border-t border-border pt-5 space-y-3">
          <button onClick={handleGlobalLogout} className="w-full py-3 rounded-xl border border-border text-sm font-medium hover:bg-muted transition-colors flex items-center justify-center gap-2"><LogOut className="w-4 h-4" /> Sign Out All Devices</button>
  ...
  const renderProfileTheme = () => (
    <motion.div key="profile-theme" initial={slideVariants.enter} animate={slideVariants.center} exit={slideVariants.exit} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
      ...
      <div className="px-4 py-6 space-y-4">
        <p className="text-sm text-muted-foreground">Choose a theme for your public profile.</p>
        <div className="space-y-3">
          {PROFILE_THEMES.map(t => (
            <button key={t.key} onClick={() => setProfileTheme(t.key)}
              className={`w-full flex items-center gap-3 p-3 rounded-2xl border transition-all ${profileTheme === t.key ? 'border-primary ring-2 ring-primary/30 bg-primary/5' : 'border-border hover:border-primary/40'}`}>
              <div className={`w-14 h-14 rounded-xl ${t.preview} flex items-center justify-center text-2xl shrink-0`}>{t.emoji}</div>
              <div className="text-left flex-1 min-w-0">
                <p className="text-sm font-semibold">{t.label}</p>
                <p className="text-xs text-muted-foreground">{t.desc}</p>
              </div>
              {profileTheme === t.key && <CheckCircle className="w-5 h-5 text-primary shrink-0" />}
            </button>
          ))}
        </div>
        <button onClick={handleSave} disabled={saving} className="w-full py-3.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg"><Save className="w-4 h-4" /> {saving ? 'Saving...' : 'Save Theme'}</button>
      </div>
    </motion.div>
  );
  ...
  const renderCreator = () => (
    <motion.div key="creator" initial={slideVariants.enter} animate={slideVariants.center} exit={slideVariants.exit} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
      ...
      <div className="px-4 py-6 space-y-5">
        ...
        <div className="space-y-2">
          ...
          {creatorManga.length === 0 ? (
            ...
            creatorManga.slice(0, 5).map(item => (
              <Link key={item.id} to={`/title/${item.slug}`} className="flex items-center justify-between border border-border rounded-xl px-4 py-3 hover:border-primary transition-colors">
        ...
        <Link to="/dashboard" className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold text-sm flex items-center justify-center gap-2 shadow-lg"><Upload className="w-4 h-4" /> Open Dashboard</Link>
      </div>
    </motion.div>
  );
      ...
      <div className="px-4 py-6 space-y-3">
        {libraryItems.length === 0 ? (
          ...
          libraryItems.map((item: any) => (
            <Link key={item.id} to={`/manhwa/${item.manga?.slug || ''}`} className="flex items-center justify-between border border-border rounded-xl px-4 py-3 hover:border-primary transition-colors">
