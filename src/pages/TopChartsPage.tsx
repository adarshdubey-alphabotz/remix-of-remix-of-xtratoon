  ...
  return (
    <div className="min-h-screen pt-24 pb-12 bg-background">
      <DynamicMeta
        ...
        description="Discover the most popular manhwa, manga, and top creators on Komixora. Rankings by views, likes, and followers. Find the best Korean manhwa and Japanese manga series."
        keywords="top manhwa, best manhwa, top manga, best manga, popular manhwa, trending manhwa, manhwa rankings, manga rankings, top webtoon creators, best manhwa 2026, most viewed manhwa, Komixora charts"
        url="https://komixora.fun/charts"
      />
              ...
              {rankedManga.map((m, i) => {
                ...
                return (
                  <motion.div key={m.id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: Math.min(i * 0.05, 0.4), duration: 0.4 }}>
                    <Link to={`/title/${m.slug}`} className={`flex items-center gap-4 p-4 border-2 ${style.border} hover:bg-primary/5 transition-all group`} style={{ boxShadow: rank <= 3 ? (style.shadow || '3px 3px 0 hsl(0 0% 8%)') : '3px 3px 0 hsl(0 0% 8%)' }}>
                      ...
                      {m.cover_url ? (
                        <img src={resolveCover(m.cover_url)!} alt={`${m.title} cover`} className="w-12 h-16 object-cover flex-shrink-0 border border-foreground/20" />
                      ) : (
                ...
                return (
                  <motion.div key={c.id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: Math.min(i * 0.05, 0.4), duration: 0.4 }}>
                    <Link to={`/publisher/${c.username || c.user_id}`} className={`flex items-center gap-4 p-4 border-2 ${style.border} hover:bg-primary/5 transition-all group`} style={{ boxShadow: rank <= 3 ? (style.shadow || '3px 3px 0 hsl(0 0% 8%)') : '3px 3px 0 hsl(0 0% 8%)' }}>
                      <div className={`w-10 h-10 flex items-center justify-center font-display text-2xl ${style.bg} ${style.text} flex-shrink-0 tracking-wider`}>{rank}</div>
                      {c.avatar_url ? (
                        <img src={c.avatar_url} alt={`${c.display_name || c.username || 'Creator'}'s avatar`} className="w-12 h-12 rounded-full object-cover flex-shrink-0 border-2 border-foreground/20" />
                      ) : (
