...
const ScheduledContentManager: React.FC<ScheduledContentManagerProps> = ({ creatorId, isAdmin = false }) => {
  ...
  return (
    <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b-2 border-foreground text-left text-muted-foreground text-xs uppercase tracking-wider">
                {isAdmin && <th className="px-4 py-3">Creator</th>}
                <th className="px-4 py-3">Title</th>
                {viewMode === 'chapters' && <th className="px-4 py-3">Chapter</th>}
                  ...
                  <tr key={item.id} className="border-b border-foreground/10 hover:bg-primary/5 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center gap-1 text-xs">
                        ...
                        {item.scheduled_at ? getTimeLabel(item.scheduled_at) : '—'}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold border border-primary text-primary hover:bg-primary hover:text-primary-foreground transition-all rounded"
                      >
      ...
      {launchConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setLaunchConfirm(null)}>
          <div className="bg-card border border-border rounded-2xl p-6 max-w-sm mx-4 space-y-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{launchConfirm.title}</p>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => setLaunchConfirm(null)} className="flex-1 px-4 py-2.5 text-sm font-semibold border border-border rounded-lg hover:bg-muted transition-colors">
                Cancel
              </button>
              <button
                onClick={handleLaunchConfirm}
                disabled={launchNowMutation.isPending}
                className="flex-1 px-4 py-2.5 text-sm font-bold bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {launchNowMutation.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
                {launchConfirm.step === 1 ? 'Yes, Launch' : '🚀 Confirm Launch'}
              </button>
            </div>
          </div>
        </div>
