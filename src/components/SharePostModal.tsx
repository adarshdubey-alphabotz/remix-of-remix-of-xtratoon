...
const SharePostModal: React.FC<SharePostModalProps> = ({ open, onClose, postId, postContent }) => {
  const [copied, setCopied] = useState(false);
  const shareUrl = `${window.location.origin}/community/post/${postId}`;
  const shareText = postContent ? postContent.slice(0, 100) : 'Check out this post!';
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
          ...
          >
            ...
            <div className="flex flex-col items-center mb-6">
              ...
              <p className="text-sm text-muted-foreground mt-1 text-center">Share this post with your community!</p>
            </div>

            {/* Copy link */}
            <div className="mb-5">
              <p className="text-sm font-medium mb-2 text-muted-foreground">Share your link</p>
              <div
                onClick={copyLink}
                className="flex items-center gap-2 px-4 py-3 bg-muted/30 border border-border rounded-xl cursor-pointer hover:bg-muted/50 transition-colors"
              >
                <span className="flex-1 text-sm truncate text-foreground">{shareUrl}</span>
            ...
            {/* Share targets */}
            <div>
              <p className="text-sm font-medium mb-3 text-muted-foreground">Share to</p>
